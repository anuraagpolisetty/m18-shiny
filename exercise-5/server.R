...
shinyServer(function(input, output) {
  ...
  output$'Womens Role Plot' <- renderPlot({
    ...
    if(input$sex == "Men"){
      plot.data <- womens.role %>% filter(sex == "Male")
    } else if (input$sex == "Women"){
      plot.data <- womens.role %>% filter(sex == "Female")
    } else{
      plot.data <- womens.role
    }
    
  
    
    #Define a y axis value based on the input Agree or Disagree
    if(input$opinion == "Agree") {
      opinion = plot.data$agree 
    } else {
      opinion = plot.data$disagree
    }
